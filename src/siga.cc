...
namespace Siga
{
    ...
    void Siga::InitDatabase(string database_name)
    {
        ...
        this->file_stream.seekg(0, this->file_stream.beg);
        for (int idx = 0; idx < this->n_estudantes; ++idx) {
            Estudante estu;
            this->file_stream.read((char *)&estu, sizeof(Estudante));

            this->rb_index.insert(IndiceMatricula(estu.ObterMatricula(),idx));
            //indiceNome.push_back(make_pair(estu.ObterNome(), idx));
        }
    }
    ...
    int Siga::PesquisaPorMatricula(int matricula)
    {
        ...
        this->rb_index.search(IndiceMatricula(matricula,90));
        //if (result != nullptr) {
        //    return result->getKey().idx;
        //}
    return -1;
    }
    ...
    //    for (const pair<string, int>& par : indiceNome)
    //    {
    //        if (par.first == nome){
    //            return par.second;
    //        }
