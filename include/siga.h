...
namespace Siga
{
class IndiceMatricula  {
    public:
        IndiceMatricula(int k, int idx): key(k), idx(idx){};
        bool operator<(IndiceMatricula &b){ return this->key <  b.key; }
        bool operator>(IndiceMatricula &b){ return this->key >  b.key; }
        bool operator==(IndiceMatricula &b){ return this->key ==  b.key;}
        int key;
        int idx;
...
class Siga
{
    ...
    private:
        ...
        fstream  file_stream;
        int      n_estudantes;
        RBTree::RedBlackTree<IndiceMatricula> rb_index;
